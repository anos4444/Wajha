...
- **Icons without a font.** Every module carries a Font Awesome Free glyph chosen for its DocType, or whatever an administrator picks in Frappe's icon picker: a Frappe icon, a glyph, or an emoji. The glyphs ship as a Desk sprite through `app_include_icons`: no web font, no CDN, and they take the theme's colours.
...
- **Table, Cards, Kanban or Report.** Every list module can be read four ways from a switch beside the search: the table, a grid of cards (photo or initials, title, subtitle, email and phone lines, badges, status), a kanban board grouped by a field, or a report that sums the list by any field with a bar of counts. Defaults come from the DocType; Shell Settings → Shell Module → *Views* refines them. Sidebar groups and Home tiles follow the business order (CRM, Selling, Buying, Stock, Manufacturing, Projects, Accounting, HR, Payroll, then tools and settings).
- **One-click language switch.** "English" on the Arabic shell, "العربية" on the English one: a chip in the header and a button in the side menu flip the user's language and reload. It sets the user's own language, so the Desk follows too.
- **A dashboard on Home, by role.** Map a role to a Frappe Dashboard in Shell Settings and its Number Cards and Dashboard Charts open the Home page for that role — KPI tiles and bar, line, pie and donut charts, drawn with the library the Desk already ships. Below it, every module flagged for the phone bar brings its own cards (leave balance, last check-in, last salary slip) under its name. With HRMS installed an "HR Overview" dashboard is seeded and mapped to HR Manager and HR User.
...
2. **Shell Module** — one record per sidebar item:
   ...
   - `icon` — Frappe's icon picker: Frappe icons, the Font Awesome glyphs (`fa-coins`, `fa-truck`, …) and an Emoji tab; blank picks one for the DocType
...
3. **Views** (optional) — on a Shell Module, the *Views* section picks the default view and the card's image, title, subtitle, extra lines, badges and the kanban field. Leave blank to let the shell derive them from the DocType.
4. **Language switch** — on by default; turn it off in Shell Settings → *Show the language switch* if users must not change their own language.
5. **Dashboards** (optional) — in Shell Settings, add a row per role: *role → Frappe Dashboard*. Build the Dashboard in the Desk from Number Cards and Dashboard Charts; the shell renders it on Home for that role. The HRMS pack seeds one for HR roles.
6. Open `/desk/wajha`.
...
MIT — see `license.txt`. The module icons are Font Awesome Free glyphs (https://fontawesome.com), used under CC BY 4.0; the attribution sits in `wajha/public/icons/fa.svg`.
