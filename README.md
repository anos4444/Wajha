...
3. **Dashboards** (optional) — in Shell Settings, add a row per role: *role → Frappe Dashboard*. Build the Dashboard in the Desk from Number Cards and Dashboard Charts; the shell renders it on Home for that role. The HRMS pack seeds one for HR roles.
4. Open `/desk/wajha`.
