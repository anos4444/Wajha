...
3. **Language switch** — on by default; turn it off in Shell Settings → *Show the language switch* if users must not change their own language.
4. **Dashboards** (optional) — in Shell Settings, add a row per role: *role → Frappe Dashboard*. Build the Dashboard in the Desk from Number Cards and Dashboard Charts; the shell renders it on Home for that role. The HRMS pack seeds one for HR roles.
5. Open `/desk/wajha`.
