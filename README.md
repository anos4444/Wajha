...
- **Table, Cards or Kanban.** Every list module can be read three ways from a switch beside the search: the table, a grid of cards (photo or initials, title, subtitle, email and phone lines, coloured badges, status), or a kanban board grouped by a field. Defaults come from the DocType; Shell Settings → Shell Module → *Views* refines them.
