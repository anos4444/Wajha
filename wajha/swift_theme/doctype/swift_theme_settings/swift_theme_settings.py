...
from wajha.swift.colour import derive_roles
...
def preset_stylesheet(slug):
    ...
    if not slug:
        return None
    if not os.path.exists(frappe.get_app_path("wajha", "public", "css", "themes", f"{slug}.css")):
        return None
    return f"/assets/wajha/css/themes/{slug}.css"
...
def _bundled_sound(event_name):
    ...
    if not filename:
        return None
    if not os.path.exists(frappe.get_app_path("wajha", "public", "sounds", filename)):
        return None
    return f"/assets/wajha/sounds/{filename}"
...
def apply_theme(theme_name):
    ...
    from wajha.swift.boot import can_switch_theme
